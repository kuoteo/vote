...
class VoteController extends Controller
{
    ...
    public function getCandidateList()
    {
        ...
        //进入队列 若队列已满 0.3s后请求
        while (!$this->vote->doQueue('Candidate', 20, 100000)) {
            usleep(100000);
        }
    ...
    /**
     * showCandidateVoteNum 展示候选人票数
     * @author leekachung <leekachung17@gmail.com>
     * @return [type] [description]
     */
    public function showCandidateVoteNum(Request $request)
    {
        
        $vote_model_id = $request->vote_model_id;

        return $this->vote->ReturnJsonResponse(
            200, $this->vote->showCandidateVoteNum($vote_model_id)
        );
    }
    ...
    public function vote(Request $request)
    {
        ...
        while (!$this->vote->doQueue('behalf', 3, 50000)) {
            usleep(50000);
        }
        ...
        try {
            ...
            //确认代表投票
            $this->behalf->checkVote($request, $auth->id);
            //提交事务
            DB::commit();
