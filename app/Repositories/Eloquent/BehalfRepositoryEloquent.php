 ...
 */
    ...
    public function exportModel(
        $filename, $sheetname, $title, $letter, $format, $content='')
    {
        Excel::create($filename, function ($excel) 
            use ($sheetname, $title, $letter, $content) {

            $excel->sheet($sheetname, function ($sheet) 
                use ($title, $letter, $content) {
 ...
 */
    ...
    {
        $data = $this->model->where(['vote_model_id' => $id])->get();
 ...
 */
     ...
     * @return [type]            [description]
     */
    public function checkVote($request, $behalf_id)
    {
        $this->model->where(['id' => $behalf_id])
            ->update([
            'is_vote' => 1,
            'candidate_id' => json_encode([
                'candidate' => $request->name,
                'recommended' => $request->other_name
            ])
        ]);
     ...
     * @return [type]            [description]
     */
    public function getVoteDetail($candidate, $id)
    {
        $list = json_decode($this->model
            ->where(['vote_model_id' => $id])->orderBy('id')->get()
        );
        ...
        foreach ($list as $key => $value) {
            if ($value->is_vote == 1) {
                $candidate_list = '';
                foreach (json_decode($value->candidate_id)->candidate
                    as $k => $v) {
                    $candidate_list .= $candidate[$v] . ',';
                }
                if (json_decode($value->candidate_id)->recommended) {
                    $recommended = implode(',', 
                        json_decode($value->candidate_id)->recommended
                    );
                } else{
                    $recommended = '';
                }
                ...
                $newArr[] = [
                    $value->name, $candidate_list . $recommended
                ];
            } else {
                $newArr[] = [
                    $value->name, '未投票'
